...
public class PlayerScript : MonoBehaviour
{
    ...
    private void move()
    {
        ...
        if (Input.GetAxis("Vertical") > 0)
        {
            ...
            Vector3 vel = transform.forward * CurrentSpeed * Input.GetAxis("Vertical");
            vel.y = rb.velocity.y; //gravity
            rb.velocity = vel;
        }
        else if (Input.GetAxis("Vertical") < 0)
        {
            CurrentSpeed = Mathf.Lerp(CurrentSpeed, MaxBackwardsSpeed, Time.deltaTime * 0.5f); //speed
            Vector3 vel = transform.forward * CurrentSpeed * Input.GetAxis("Vertical");
            vel.y = rb.velocity.y; //gravity
            rb.velocity = vel;
        }
    ...
    private void tireSteer()
    {
        ...
        {
            frontLeftTire.localEulerAngles = Vector3.Lerp(frontLeftTire.localEulerAngles, new Vector3(0, 0, 335), 5 * Time.deltaTime);
            frontRightTire.localEulerAngles = Vector3.Lerp(frontLeftTire.localEulerAngles, new Vector3(0, 0, 335), 5 * Time.deltaTime);
        }
        else if (Input.GetAxis("Horizontal") > 0)
        {
            frontLeftTire.localEulerAngles = Vector3.Lerp(frontLeftTire.localEulerAngles, new Vector3(0, 0, 385), 5 * Time.deltaTime);
            frontRightTire.localEulerAngles = Vector3.Lerp(frontLeftTire.localEulerAngles, new Vector3(0, 0, 385), 5 * Time.deltaTime);
        }
        else
        {
            frontLeftTire.localEulerAngles = Vector3.Lerp(frontLeftTire.localEulerAngles, new Vector3(0, 0, 360), 5 * Time.deltaTime);
            frontRightTire.localEulerAngles = Vector3.Lerp(frontLeftTire.localEulerAngles, new Vector3(0, 0, 360), 5 * Time.deltaTime);
        }
